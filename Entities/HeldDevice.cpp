	...
	if (!IsUnPickupable()) {
		if (m_Parent) {
			...
			if (pSymbolFont && pTextFont) {
				...
				m_SeenByPlayer.at(viewingPlayer) = unheldItemDisplayRange < 0 || (unheldItemDisplayRange > 0 && g_SceneMan.ShortestDistance(m_Pos, g_SceneMan.GetScrollTarget(whichScreen), g_SceneMan.SceneWrapsX()).IsMagnitudeLessThan(unheldItemDisplayRange));
