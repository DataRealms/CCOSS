...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	void TerrainDebris::ApplyDebris(SLTerrain *terrain) {
		...
		for (unsigned int piece = 0; piece < pieceCount; ++piece) {
			bool place = false;
			unsigned short currentBitmap = std::floor(static_cast<float>(m_BitmapCount) * PosRand());
			...
			int x = std::floor(static_cast<float>(terrainWidth) * PosRand());
			int y = 0;
			int depth = std::floor(RangeRand(m_MinDepth, m_MaxDepth));
