...
{
    for( vector<DataModule *>::iterator dmItr = m_pDataModules.begin(); dmItr != m_pDataModules.end();
         ++dmItr ) {
        delete( *dmItr );
    }
...
bool PresetMan::LoadDataModule( string moduleName, bool official,
                                ProgressCallback fpProgressCallback )
{
    if( moduleName.empty() ) {
        return false;
    }
...
bool PresetMan::LoadAllDataModules()
{
    ...
    if( !m_SingleModuleToLoad.empty() &&
        std::find( officialModules.begin(), officialModules.end(),
                   m_SingleModuleToLoad ) == officialModules.end() ) {
        if( !LoadDataModule( m_SingleModuleToLoad, false, &LoadingScreen::LoadingSplashProgressReport ) ) {
            g_ConsoleMan.PrintString( "ERROR: Failed to load DataModule \"" + m_SingleModuleToLoad +
                                      "\"! Only official modules were loaded!" );
            return false;
        }
    } else {
        std::vector<std::filesystem::directory_entry> workingDirectoryFolders;
        std::copy_if( std::filesystem::directory_iterator( System::GetWorkingDirectory() ),
                      std::filesystem::directory_iterator(), std::back_inserter( workingDirectoryFolders ),
        []( auto dirEntry ) {
        ...
        for( const std::filesystem::directory_entry &directoryEntry : workingDirectoryFolders ) {
            ...
            if( std::regex_match( directoryEntryPath, std::regex( ".*\.rte" ) ) ) {
                std::string moduleName = directoryEntryPath.substr( directoryEntryPath.find_last_of( '/' ) + 1,
                                         std::string::npos );
                if( !g_SettingsMan.IsModDisabled( moduleName ) &&
                    ( std::find( officialModules.begin(), officialModules.end(),
                                 moduleName ) == officialModules.end() && moduleName != "Metagames.rte" &&
                      moduleName != "Scenes.rte" ) ) {
                    ...
                    if( moduleID < 0 || moduleID >= GetOfficialModuleCount() ) {
                        LoadDataModule( moduleName, false, &LoadingScreen::LoadingSplashProgressReport );
                    }
                }
            }
        }
    ...
    if( g_SettingsMan.IsMeasuringModuleLoadTime() ) {
        std::chrono::milliseconds moduleLoadElapsedTime =
            std::chrono::duration_cast<std::chrono::milliseconds>( std::chrono::high_resolution_clock::now() -
                    moduleLoadTimerStart );
        g_ConsoleMan.PrintString( "Module load duration is: " + std::to_string(
                                      moduleLoadElapsedTime.count() ) + "ms" );
    }
    return true;
}
...
{
    RTEAssert( whichModule < ( int )m_pDataModules.size(),
...
bool PresetMan::AddEntityPreset( Entity *pEntToAdd, int whichModule, bool overwriteSame,
                                 string readFromFile )
{
    RTEAssert( whichModule >= 0 &&
               whichModule < m_pDataModules.size(), "Tried to access an out of bounds data module number!" );
...
const Entity *PresetMan::GetEntityPreset( string type, string preset, int whichModule )
{
    ...
    // Specific module
        ...
        if( !pRetEntity ) {
            RTEAssert( m_OfficialModuleCount <= m_pDataModules.size(),
                       "More official modules than modules loaded?!" );
...
const Entity *PresetMan::GetEntityPreset( Reader &reader )
{
    ...
    RTEAssert( whichModule >= 0 &&
               whichModule < m_pDataModules.size(), "Reader has an out of bounds module number!" );
    ...
    if( pClass && pClass->IsConcrete() ) {
        ...
        if( pNewInstance && pNewInstance->Create( reader, false ) < 0 ) {
            ...
            if( !g_PresetMan.GetDataModule( whichModule )->GetIgnoreMissingItems() ) {
                RTEAbort( "Reading of a preset instance \"" + pNewInstance->GetPresetName() + "\" of class " +
                          pNewInstance->GetClassName() + " failed in file " + reader.GetCurrentFilePath() +
                          ", shortly before line #" + reader.GetCurrentFileLine() );
            }
        } else if( pNewInstance ) {
            // Try to add the instance to the collection
            m_pDataModules[whichModule]->AddEntityPreset( pNewInstance, reader.GetPresetOverwriting(),
                    entityFilePath );

            // Regardless of whether there was a collision or not, use whatever now exists in the instance map of that class and name
            pReturnPreset = m_pDataModules[whichModule]->GetEntityPreset( pNewInstance->GetClassName(),
                            pNewInstance->GetPresetName() );
            ...
            if( !pReturnPreset ) {
                ...
                for( int i = 0; i < m_OfficialModuleCount && !pReturnPreset; ++i ) {
                    pReturnPreset = m_pDataModules[i]->GetEntityPreset( pNewInstance->GetClassName(),
                                    pNewInstance->GetPresetName() );
                }
            }
        }
...
bool PresetMan::GetAllOfType( list<Entity *> &entityList, string type, int whichModule )
{
    ...
    // Specific module
        ...
        RTEAssert( whichModule < m_pDataModules.size(),
                   "Trying to get from an out of bounds DataModule ID!" );
...
bool PresetMan::GetAllOfTypeInModuleSpace( std::list<Entity *> &entityList, std::string type,
        int whichModuleSpace )
{
    if( type.empty() ) {
        return false;
    }
...
bool PresetMan::GetAllOfGroup( list<Entity *> &entityList, string group, string type,
                               int whichModule )
{
    ...
    if( whichModule < 0 ) {
        for( DataModule *dataModule : m_pDataModules ) {
            if( dataModule->IsFaction() ) {
                foundAny = dataModule->GetAllOfGroup( tempList, group, type ) || foundAny;
            }
        }
    } else {
        RTEAssert( whichModule < m_pDataModules.size(),
    ...
    if( foundAny ) {
        ...
        if( group == "Brains" ) {
            ...
            for( list<Entity *>::iterator oItr = tempList.begin(); oItr != tempList.end(); ++oItr ) {
                ...
                if( pSObject && pSObject->IsBuyable() && !pSObject->IsBuyableInObjectPickerOnly() &&
                    !pSObject->IsInGroup( "Brains" ) ) {
                    entityList.push_back( *oItr );
                    foundAny = true;
                }
            }
...
bool PresetMan::GetAllOfGroupInModuleSpace( list<Entity *> &entityList, string group, string type,
        int whichModuleSpace )
{
    RTEAssert( !group.empty(), "Looking for empty group!" );
...
{
    RTEAssert( whichModule >= m_OfficialModuleCount &&
               whichModule < m_pDataModules.size(),
               "Tried to make a material mapping in an offical or out-of-bounds DataModule!" );
...
{
    RTEAssert( whichModule >= 0 &&
               whichModule < ( int )m_pDataModules.size(),
               "Tried to access an out of bounds data module number!" );
...
bool PresetMan::GetGroups( list<string> &groupList, int whichModule, string withType ) const
{
    ...
    if( whichModule < 0 ) {
        ...
        if( withType == "All" || withType.empty() ) {
            for( list<string>::const_iterator gItr = m_TotalGroupRegister.begin();
...
bool PresetMan::GetModuleSpaceGroups( list<string> &groupList, int whichModule,
                                      string withType ) const
{
    RTEAssert( whichModule < ( int )m_pDataModules.size(),
               "Tried to access an out of bounds data module number!" );
...
Actor *PresetMan::GetLoadout( std::string loadoutName, int moduleNumber, bool spawnDropShip )
{
    if( spawnDropShip ) {
        ...
        const Loadout *pLoadout = dynamic_cast<const Loadout *>( g_PresetMan.GetEntityPreset( "Loadout",
                                  loadoutName, moduleNumber ) );
        if( pLoadout ) {
...
void PresetMan::FindAndExtractZippedModules() const
{
    for( const std::filesystem::directory_entry &directoryEntry : std::filesystem::directory_iterator(
        ...
        if( zippedModulePath.find( System::GetZippedModulePackageExtension() ) == zippedModulePath.length()
            - System::GetZippedModulePackageExtension().length() ) {
            LoadingScreen::LoadingSplashProgressReport( "Extracting Data Module from: " +
                    directoryEntry.path().filename().generic_string(), true );
            LoadingScreen::LoadingSplashProgressReport( System::ExtractZippedDataModule( zippedModulePath ),
                    true );
        }
    }
}
