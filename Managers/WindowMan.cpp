...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	void WindowMan::CreateTextures() {
		if (m_MultiDisplayTextureOffsets.empty()) {
			m_ScreenTexture = std::unique_ptr<SDL_Texture, SDLTextureDeleter>(SDL_CreateTexture(m_Renderer.get(), SDL_PIXELFORMAT_ABGR8888, SDL_TEXTUREACCESS_STREAMING, m_ResX, m_ResY));
			SDL_RenderSetLogicalSize(m_Renderer.get(), m_ResX, m_ResY);
		} else {
			...
			for (size_t i = 0; i < m_MultiDisplayTextures.size(); ++i) {
				...
				m_MultiDisplayTextures[i] = std::unique_ptr<SDL_Texture, SDLTextureDeleter>(
					SDL_CreateTexture(renderer, SDL_PIXELFORMAT_ABGR8888, SDL_TEXTUREACCESS_STREAMING, m_MultiDisplayTextureOffsets[i].w, m_MultiDisplayTextureOffsets[i].h));
				if (!m_MultiDisplayTextures[i]) {
					RTEAbort("Failed to create texture for multi-display: " + std::string(SDL_GetError()));
				}
				SDL_RenderSetLogicalSize(renderer, m_MultiDisplayTextureOffsets[i].w, m_MultiDisplayTextureOffsets[i].h);
			}
		}
	}
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	void WindowMan::ChangeResolution(int newResX, int newResY, bool upscaled, bool newFullscreen) {
		...
		if (newFullscreen && ((m_NumScreens > 1 && !SetWindowMultiFullscreen(newResX, newResY, newResMultiplier)) || SDL_SetWindowFullscreen(m_Window.get(), SDL_WINDOW_FULLSCREEN_DESKTOP) != 0)) {
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	void WindowMan::ChangeResolutionMultiplier() {
		...
		if (m_ResX * newResMultiplier == m_MaxResX && m_ResY * newResMultiplier == m_MaxResY) {
			if ((m_NumScreens > 1 && !SetWindowMultiFullscreen(m_ResX, m_ResY, newResMultiplier)) || SDL_SetWindowFullscreen(m_Window.get(), SDL_WINDOW_FULLSCREEN_DESKTOP) != 0) {
				if (!AttemptToRevertToPreviousResolution()) {
			...
			SDL_RestoreWindow(m_Window.get());
			SDL_SetWindowBordered(m_Window.get(), SDL_TRUE);
			SDL_SetWindowSize(m_Window.get(), m_ResX * newResMultiplier, m_ResY * newResMultiplier);
			int displayIndex = SDL_GetWindowDisplayIndex(m_Window.get());
			SDL_SetWindowPosition(m_Window.get(), SDL_WINDOWPOS_CENTERED_DISPLAY(displayIndex), SDL_WINDOWPOS_CENTERED_DISPLAY(displayIndex));
		}
		m_ResMultiplier = newResMultiplier;
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	bool WindowMan::AttemptToRevertToPreviousResolution() const {
		...
		return SDL_SetWindowFullscreen(m_Window.get(), IsFullscreen() ? SDL_WINDOW_FULLSCREEN_DESKTOP : 0) != 0;
	}
	...
	void WindowMan::UploadFrame() {
		...
		if (m_MultiDisplayTextureOffsets.empty()) {
			...
			for (size_t i = 0; i < m_MultiDisplayTextureOffsets.size(); ++i) {
				...
				SDL_UpdateTexture(m_MultiDisplayTextures[i].get(), NULL, g_FrameMan.GetBackBuffer32()->line[0] + (m_MultiDisplayTextureOffsets[i].x + g_FrameMan.GetBackBuffer32()->w * m_MultiDisplayTextureOffsets[i].y) * 4, g_FrameMan.GetBackBuffer32()->w * 4);
