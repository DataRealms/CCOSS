...
void MainMenuGUI::UpdateResolutionCombo() {
    ...
    if (m_pResolutionCombo->GetCount() <= 0) {
		...
		if (currentResIndex < 0) {
			std::snprintf(resString, sizeof(resString), "%ix%i Custom", g_FrameMan.GetNewResX() * g_FrameMan.ResolutionMultiplier(), g_FrameMan.GetNewResY() * g_FrameMan.ResolutionMultiplier());
			m_pResolutionCombo->AddItem(resString);
