...
//        HeldDevice *pDevice = 0;
        for (list<SceneObject *>::const_iterator itr = pSceneObjectList->begin(); itr != pSceneObjectList->end(); ++itr, ++i)
        {
            ...
            {
                g_FrameMan.SetTransTableFromPreset(m_BlinkTimer.AlternateReal(333) || m_EditorGUIMode == PLACINGOBJECT ? TransparencyPreset::Trans25 : TransparencyPreset::Trans50);
...
//                    if (m_BlinkTimer.AlternateReal(1000))
                    ...
                    if (i >= m_RevealIndex)
                    {
                        g_FrameMan.SetTransTableFromPreset(pActor ? TransparencyPreset::Trans75 : TransparencyPreset::Trans50);
                        (*itr)->Draw(pTargetBitmap, targetPos, g_DrawTrans);
                    }
