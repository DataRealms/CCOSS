...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	int DataModule::Create(std::string moduleName, ProgressCallback progressCallback) {
		...
		if (std::experimental::filesystem::exists(indexPath.c_str()) && reader.Create(indexPath.c_str(), true, progressCallback) >= 0) {
			...
			if (m_ScanFolderContents) {
				...
				for (int result = al_findfirst(searchPath.c_str(), &fileInfo, FA_ALL); result == 0; result = al_findnext(&fileInfo)) {
					...
					if (std::strlen(fileInfo.name) > 0 && std::string(fileInfo.name) != "Index.ini") {
						...
						if (std::experimental::filesystem::exists(iniPath.c_str()) && iniReader.Create(iniPath.c_str(), false, progressCallback) >= 0) {
