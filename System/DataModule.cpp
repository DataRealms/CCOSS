...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	int DataModule::Create(const std::string &moduleName, const ProgressCallback &progressCallback) {
		...
		if (std::filesystem::exists(indexPath) && reader.Create(indexPath, true, progressCallback) >= 0) {
			...
			if (m_ScanFolderContents) {
				...
				for (int result = al_findfirst(searchPath.c_str(), &fileInfo, FA_ALL); result == 0; result = al_findnext(&fileInfo)) {
					...
					if (std::strlen(fileInfo.name) > 0 && std::string(fileInfo.name) != "Index.ini") {
						...
						if (std::filesystem::exists(iniPath) && iniReader.Create(iniPath, false, progressCallback) >= 0) {
