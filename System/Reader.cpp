...
int Reader::Create(const char *filename, bool overwrites, void (*fpProgressCallback)(std::string, bool), bool failOK)
{
    ...
    if (lastSlashPos == string::npos)
        lastSlashPos = m_FilePath.find_last_of('\\');
    ...
    if (firstSlashPos == string::npos)
        firstSlashPos = m_FilePath.find_first_of('\\');
...
string Reader::ReadPropValue()
{
    ...
    string subStr = (begin == string::npos ? fullLine : fullLine.substr(begin + 1));
