...
//		From this template, we intstantiate two classes:  StdStringA and
//		StdStringW.  The name "StdString" is just a #define of one of these,
...
//				  - Added a specialization of std::swap for StdString.  Thanks
...
//					ability to build StdString on Sun Unix systems.  He
//					sent me detailed build reports about what works and what
//					does not.  If StdString compiles on your Unix box, you
...
// 2000-FEB-01 - You can now use StdString on the Mac with CodeWarrior!
...
//					with my implementation of StdString::FormatV in which
...
//					bug that MS has not fixed.  StdString did nothing to fix
...
//					The resulting string would be empty.  Not with StdString!
...
//	  1999-JUL-01 - Added a true CString facade.  Now you can use StdString as
...
//					to a CArchive and later read it in as a StdString.   I have
//					no idea why you would want to do this but you can. 

//	  1999-JUN-21 - Changed the StdString class into the CStdStr template.
...
//				  - new sscpy() replacement of StdString::CopyString()
...
//					persisting StdStrings to/from DCOM IStream interfaces 
//				  - Added functional objects (e.g. StdStringLessNoCase) that
//					allow StdStrings to be used as keys STL map objects with
...
// Turn off unavoidable compiler warnings
	...
	#pragma once
	#pragma component(browser, off, references, "StdString")
...
// arguments than the standard dictates.  Since I'm trying to write StdString
...
public:
	...
	//		void _cdecl Formst(StdStringA &PCSTR szFormat, ...)
	//		void _cdecl Format(PCSTR szFormat);
	// 
	// DESCRIPTION:
	//		This function does sprintf/wsprintf style formatting on StdStringA
	...
	MYTYPE SpanIncluding(PCMYSTR szCharSet) const
	{
		...
		// that StdStringW gets instantiated even in non-Unicode builds.
...
//	void			UnlockBuffer(); { }	// why have UnlockBuffer w/o LockBuffer?
	...
	// FUNCTION: StreamSize
	// REMARKS:
	//		Returns how many bytes it will take to StreamSave() this StdString
	...
	// FUNCTION: StreamSave
	// REMARKS:
	//		Saves this StdString object to a COM IStream.
	// -------------------------------------------------------------------------
...
// StdString to build on Sun Unix systems.
// -----------------------------------------------------------------------------
...
typedef CStdStr<char>		StdStringA;	// a better std::string
typedef CStdStr<wchar_t>	StdStringW;	// a better std::wstring
typedef CStdStr<OLECHAR>	StdStringO;	// almost always StdStringW
...
// If you want to export StdStringA and StdStringW from a DLL, then all you
// need to
...
// Special operators that allow us to serialize StdStrings to CArchives.
// Note that we use an intermediate CString object in order to ensure that
// we use the exact same format.

#ifdef _MFC_VER
	inline CArchive &AFXAPI operator<<(CArchive &ar, const StdStringA &strA)
	{
		CString strTemp	= strA;
		return ar << strTemp;
	}
	inline CArchive &AFXAPI operator<<(CArchive &ar, const StdStringW &strW)
	{
		CString strTemp	= strW;
		return ar << strTemp;
	}

	inline CArchive &AFXAPI operator>>(CArchive &ar, StdStringA &strA)
	{
		CString strTemp;
		ar >> strTemp;
		strA = strTemp;
	...
	inline CArchive &AFXAPI operator>>(CArchive &ar, StdStringW &strW)
	{
		CString strTemp;
		ar >> strTemp;
		strW = strTemp;
...
//		StdStringA WUFormat(UINT nId, ...);
//		StdStringA WUFormat(PCSTR szFormat, ...);

// REMARKS:
//		This function allows the caller for format and return a StdStringA
//		object with a single line of code.
// -----------------------------------------------------------------------------
#ifdef SS_ANSI
#else
	inline StdStringA WUFormatA(UINT nId, ...)
	{
		va_list argList;
		va_start(argList, nId);
	...
	inline StdStringA WUFormatA(PCSTR szFormat, ...)
	{
		va_list argList;
		va_start(argList, szFormat);
		StdStringA strOut;
	...
	inline StdStringW WUFormatW(UINT nId, ...)
	{
		va_list argList;
		va_start(argList, nId);
	...
	inline StdStringW WUFormatW(PCWSTR szwFormat, ...)
	{
		va_list argList;
	...
	//	 StdStringA WUSysMessageA(DWORD dwError, DWORD dwLangId=SS_DEFLANGID);
	//	 StdStringW WUSysMessageW(DWORD dwError, DWORD dwLangId=SS_DEFLANGID);
	// 
	// DESCRIPTION:
	...
	//	 corresponding system string is returned in the form of a StdStringA.
	...
	// RETURN VALUE: 
	//	 a StdStringA equivalent of the error code.  Currently, this function
	...
	inline StdStringA WUSysMessageA(DWORD dwError, DWORD dwLangId=SS_DEFLANGID)
	{
		CHAR szBuf[512];
	...
	inline StdStringW WUSysMessageW(DWORD dwError, DWORD dwLangId=SS_DEFLANGID)
	{
		WCHAR szBuf[512];
...
//		StdString objects.  This is useful for maps in which the key may be the
...
struct StdStringLessNoCaseW
	: std::binary_function<StdStringW, StdStringW, bool>
{
	inline
	bool operator()(const StdStringW &sLeft, const StdStringW &sRight) const
...
struct StdStringLessNoCaseA
	: std::binary_function<StdStringA, StdStringA, bool>
{
	inline
	bool operator()(const StdStringA &sLeft, const StdStringA &sRight) const
	{ return ssicmp(sLeft.c_str(), sRight.c_str()) < 0; }
};
...
//namespace std
//{
//	inline void swap(StdStringA &s1, StdStringA &s2) throw()
//	{
//		s1.swap(s2);
//	}
//	template<>
//	inline void swap(StdStringW &s1, StdStringW &s2) throw()
//	{
//		s1.swap(s2);
//	}
//}
