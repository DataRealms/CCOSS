...
PACKFILE *pack_fopen_chunk(PACKFILE *f, int pack)
{
   ...
   if (f->normal.flags & PACKFILE_FLAG_WRITE) {
	 ...
	 if (f->normal.passdata) {
	    ...
	    chunk->normal.passpos = chunk->normal.passdata + (ptrdiff_t)f->normal.passpos - (ptrdiff_t)f->normal.passdata;
	    f->normal.passpos = f->normal.passdata;
	 }
...
PACKFILE *pack_fclose_chunk(PACKFILE *f)
{
   ...
   if (f->normal.flags & PACKFILE_FLAG_WRITE) {
	 ...
	 parent->normal.passpos = parent->normal.passdata + (ptrdiff_t)f->normal.passpos - (ptrdiff_t)f->normal.passdata;
