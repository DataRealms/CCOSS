   ...
   switch (msg) {
	 ...
	 if (listsize) {
	    if (c >= ' ') {
		  ...
		  if ((int)((uintptr_t)d->dp3) < ustrlen(thisitem)) {
		     for (a=0; a < (int)((uintptr_t)d->dp3); a++) {
			if (utolower(ugetat(thisitem, a)) != utolower(ugetat(selected, a))) {
		     ...
		     if ((!failure) && (utolower(ugetat(thisitem, (int)(uintptr_t)d->dp3)) == utolower(c))) {
			d->d1 = i;
			d->dp3 = (void *)((uintptr_t)d->dp3 + 1);
