...
static void ddraw_blit_to_self(BITMAP * source, BITMAP * dest, int source_x, int source_y, int dest_x, int dest_y, int width, int height)
{
   ...
   IDirectDrawSurface7_BltFast(DDRAW_SURFACE_OF(dest_parent)->id, dest_parent_x, dest_parent_y,
                               DDRAW_SURFACE_OF(source_parent)->id, &src_rect,
                               DDBLTFAST_WAIT);
   _exit_gfx_critical();
   
   /* only for windowed mode */
   if ((gfx_driver->id == GFX_DIRECTX_WIN || gfx_driver->id == GFX_DIRECTX_WIN_BORDERLESS) && (dest_parent == gfx_directx_forefront_bitmap)) {
...
static void ddraw_masked_blit(BITMAP * source, BITMAP * dest, int source_x, int source_y, int dest_x, int dest_y, int width, int height)
{
   ...
   if (is_video_bitmap(source) || is_system_bitmap(source)) {
      ...
      IDirectDrawSurface7_SetColorKey(DDRAW_SURFACE_OF(source_parent)->id,
                                      DDCKEY_SRCBLT, &src_key);

      hr = IDirectDrawSurface7_Blt(DDRAW_SURFACE_OF(dest_parent)->id, &dest_rect,
...
static void ddraw_clear_to_color(BITMAP * bitmap, int color)
{
   ...
   hr = IDirectDrawSurface7_Blt(DDRAW_SURFACE_OF(parent)->id, &dest_rect,
   ...
   /* only for windowed mode */
   if ((gfx_driver->id == GFX_DIRECTX_WIN || gfx_driver->id == GFX_DIRECTX_WIN_BORDERLESS) && (parent == gfx_directx_forefront_bitmap))
      win_gfx_driver->paint(&dest_rect);
}
...
void gfx_directx_enable_triple_buffering(GFX_DRIVER *drv)
{
   ...
   hr = IDirectDrawSurface7_GetFlipStatus(DDRAW_SURFACE_OF(gfx_directx_forefront_bitmap)->id, DDGFS_ISFLIPDONE);
