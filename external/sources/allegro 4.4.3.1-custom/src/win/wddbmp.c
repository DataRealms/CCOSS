 ...
 */
static LPDIRECTDRAWSURFACE7 create_directdraw7_surface(int w, int h, LPDDPIXELFORMAT pixel_format, int type, int n_backbuffers)
{
   DDSURFACEDESC2 ddsurf_desc;
   LPDIRECTDRAWSURFACE7 ddsurf1;
   LPVOID ddsurf2;
   HRESULT hr;

   /* describe surface characteristics */
   memset(&ddsurf_desc, 0, sizeof(DDSURFACEDESC2));
   ddsurf_desc.dwSize = sizeof(DDSURFACEDESC2);
   ...
   /* create the surface */
   hr = IDirectDraw7_CreateSurface(directdraw, &ddsurf_desc, &ddsurf1, NULL);
   if (FAILED(hr)) {
   ...
   hr = IDirectDrawSurface7_QueryInterface(ddsurf1, &IID_IDirectDrawSurface7, &ddsurf2);
    ...
    */
   if (_dx_ver >= 0x500)
      IDirectDrawSurface7_Release(ddsurf1);
   ...
   return (LPDIRECTDRAWSURFACE7)ddsurf2;
}
...
DDRAW_SURFACE *gfx_directx_create_surface(int w, int h, LPDDPIXELFORMAT pixel_format, int type)
{
   ...
   surf->id = create_directdraw7_surface(w, h, pixel_format,type, 0);
   if (!surf->id) {
      _AL_FREE(surf);
      return NULL;
   }
...
{
   IDirectDrawSurface7_Release(surf->id);
...
static int recreate_flipping_chain(int n_pages)
{
   ...
   if (flipping_page[0]->id) {
      hr = IDirectDrawSurface7_Release(flipping_page[0]->id);
      if (FAILED(hr)) {
   ...
   flipping_page[0]->id = create_directdraw7_surface(w, h, ddpixel_format, type, n_backbuffers);
   if (!flipping_page[0]->id) 
      return -1;
   ...
   if (n_backbuffers > 0) {
      ...
      hr = IDirectDrawSurface7_GetAttachedSurface(flipping_page[0]->id, &ddscaps, &flipping_page[1]->id);
      if (FAILED(hr)) {
      ...
      if (n_backbuffers > 1) {
         ...
         hr = IDirectDrawSurface7_GetAttachedSurface(flipping_page[1]->id, &ddscaps, &flipping_page[2]->id);
         if (FAILED(hr)) {
   ...
   if (flipping_page[0]->flags & DDRAW_SURFACE_INDEXED) {
      hr = IDirectDrawSurface7_SetPalette(flipping_page[0]->id, ddpalette);
      if (FAILED(hr)) {
...
static int flip_with_forefront_bitmap(BITMAP *bmp, int wait)
{
   ...
   hr = IDirectDrawSurface7_Flip(flipping_page[0]->id, surf->id, wait ? DDFLIP_WAIT : 0);
...
int gfx_directx_poll_scroll(void)
{
   ...
   hr = IDirectDrawSurface7_GetFlipStatus(flipping_page[0]->id, DDGFS_ISFLIPDONE);
...
HDC win_get_dc(BITMAP *bmp)
{
   ...
   if (bmp) {
      if (bmp->id & (BMP_ID_SYSTEM | BMP_ID_VIDEO)) {
         ...
         hr = IDirectDrawSurface7_GetDC(ddsurf, &dc);
...
void win_release_dc(BITMAP *bmp, HDC dc)
{
   ...
   if (bmp) {
      if (bmp->id & (BMP_ID_SYSTEM | BMP_ID_VIDEO)) {
         ddsurf = DDRAW_SURFACE_OF(bmp)->id;
         hr = IDirectDrawSurface7_ReleaseDC(ddsurf, dc);
