...
static int wnd_set_video_mode(void)
{
   ...
   hr = IDirectDraw7_SetCooperativeLevel(directdraw, allegro_wnd, DDSCL_EXCLUSIVE | DDSCL_FULLSCREEN);
   if (FAILED(hr)) {
   ...
   hr = IDirectDraw7_SetDisplayMode(directdraw, _wnd_width, _wnd_height, _wnd_depth,
...
int gfx_directx_compare_color_depth(int color_depth)
{
   ...
   hr = IDirectDraw7_GetDisplayMode(directdraw, &surf_desc);
   if (FAILED(hr)) {
...
int gfx_directx_update_color_format(DDRAW_SURFACE *surf, int color_depth)
{
   ...
   hr = IDirectDrawSurface7_GetPixelFormat(surf->id, &pixel_format);
   if (FAILED(hr)) {
      _TRACE(PREFIX_E "Can't get color format.\n");
      return -1;
   }
 ...
 */ 
static HRESULT CALLBACK EnumModesCallback(LPDDSURFACEDESC2 lpDDSurfaceDesc, LPVOID gfx_mode_list_ptr)
{
...
GFX_MODE_LIST *gfx_directx_fetch_mode_list(void)
{
   ...
   hr = IDirectDraw7_EnumDisplayModes(directdraw, enum_flags, NULL, gfx_mode_list, EnumModesCallback);
   if (FAILED(hr)) {
