...
int init_directx(void)
{
   ...
   hr = IDirectDraw_QueryInterface(directdraw1, &IID_IDirectDraw7, &temp);
   if (FAILED(hr))
      return -1;
   ...
   hr = IDirectDraw7_GetCaps(directdraw, &ddcaps, NULL);
   if (FAILED(hr)) {
...
int gfx_directx_create_clipper(HWND hwnd)
{
   ...
   hr = IDirectDraw7_CreateClipper(directdraw, 0, &ddclipper, NULL);
   if (FAILED(hr)) {
...
int gfx_directx_create_palette(DDRAW_SURFACE *surf)
{
   ...
   hr = IDirectDraw7_CreatePalette(directdraw, DDPCAPS_8BIT | DDPCAPS_ALLOW256, palette_entry, &ddpalette, NULL);
   if (FAILED(hr)) {
      _TRACE(PREFIX_E "Can't create palette (%x)\n", hr);
      return -1;
   }

   hr = IDirectDrawSurface7_SetPalette(surf->id, ddpalette);
     if (FAILED(hr)) {
...
int gfx_directx_setup_driver(GFX_DRIVER *drv, int w, int h, int color_depth)
{
   ...
   IDirectDraw7_GetAvailableVidMem(directdraw, &ddsCaps, (unsigned long*)&drv->vid_mem, NULL);
...
int finalize_directx_init(void)
{
   ...
   hr = IDirectDraw7_GetMonitorFrequency(directdraw, &freq);
...
int exit_directx(void)
{
   if (directdraw) {      
      ...
      IDirectDraw7_SetCooperativeLevel(directdraw, win_get_window(), DDSCL_NORMAL);
...
{
   IDirectDraw7_WaitForVerticalBlank(directdraw, DDWAITVB_BLOCKBEGIN, NULL);
}
