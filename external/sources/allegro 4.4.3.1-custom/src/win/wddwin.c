...
static BITMAP *init_directx_win_borderless(int w, int h, int v_w, int v_h, int color_depth);
...
GFX_DRIVER gfx_directx_win_borderless =
{
   GFX_DIRECTX_WIN_BORDERLESS,
   empty_string,
   empty_string,
   "DirectDraw winbrd",
   init_directx_win_borderless,
   gfx_directx_win_exit,
   NULL,                        // AL_METHOD(int, scroll, (int x, int y)); 
   gfx_directx_sync,
   gfx_directx_set_palette,
...
static void paint_win_borderless(RECT *);
...
static WIN_GFX_DRIVER win_gfx_driver_windowed_borderless =
{
   TRUE,
   switch_in_win,
   NULL,                        // AL_METHOD(void, switch_out, (void));
...
static void paint_win(RECT *rect)
{
    ...
    */
   if (IDirectDrawSurface7_IsLost(gfx_directx_primary_surface->id))
      switch_in_win();
 ...
 *  Handles window paint events.
 */
static void paint_win_borderless(RECT *rect)
{
   /* we may have lost the DirectDraw surfaces
    * (e.g after the monitor has gone to low power)
    */
   ...
   /* clip the rectangle */
   rect->right = MIN(rect->right, gfx_directx_win_borderless.w);
   rect->bottom = MIN(rect->bottom, gfx_directx_win_borderless.h);
...
static void update_matching_window(RECT *rect)
{
   ...
   IDirectDrawSurface7_Blt(gfx_directx_primary_surface->id, &dest_rect.r,
 ...
 *  Updates a portion of the window when the color depths match.
 */
static void update_matching_window_borderless(RECT *rect)
{
   HWND allegro_wnd = win_get_window();
   union {
     POINT p;
   ...
   if (rect)
      ...
      dest_rect.r.left   = 0;
      dest_rect.r.right  = gfx_directx_win_borderless.w;
      dest_rect.r.top    = 0;
      dest_rect.r.bottom = gfx_directx_win_borderless.h;
   }
 ...
 */
static int ddsurf_blit_ex(LPDIRECTDRAWSURFACE7 dest_surf, RECT *dest_rect,
                          LPDIRECTDRAWSURFACE7 src_surf, RECT *src_rect)
{
   DDSURFACEDESC2 src_desc, dest_desc;
   ...
   hr = IDirectDrawSurface7_Lock(dest_surf, dest_rect, &dest_desc,
                                 DDLOCK_WAIT | DDLOCK_SURFACEMEMORYPTR, NULL);
   if (FAILED(hr))
      return -1;

   hr = IDirectDrawSurface7_Lock(src_surf, src_rect, &src_desc,
                                 DDLOCK_WAIT | DDLOCK_SURFACEMEMORYPTR, NULL);
   if (FAILED(hr)) {
      IDirectDrawSurface7_Unlock(dest_surf, NULL);
      return -1;
   }
   ...
   IDirectDrawSurface7_Unlock(src_surf, NULL);
   IDirectDrawSurface7_Unlock(dest_surf, NULL);
...
static void update_colorconv_window(RECT *rect)
{
   ...
   if (direct) {
      ...
      hr = IDirectDrawSurface7_GetDC(offscreen_surface->id, &src_dc);
      if (FAILED(hr))
         goto End;

      dest_dc = GetDC(allegro_wnd);
      if (!dest_dc) {
         IDirectDrawSurface7_ReleaseDC(offscreen_surface->id, src_dc);
         goto End;
      }
 ...
 *  Updates a portion of the window when the color depths don't match.
 */
static void update_colorconv_window_borderless(RECT *rect)
{
   RECT src_rect;
   union {
     POINT p;
   ...
   if (rect) {
      ...
      src_rect.left   = 0;
      src_rect.right  = gfx_directx_win_borderless.w;
      src_rect.top    = 0;
      src_rect.bottom = gfx_directx_win_borderless.h;
   }
...
static int wnd_set_windowed_coop(void)
{
   ...
   hr = IDirectDraw7_SetCooperativeLevel(directdraw, allegro_wnd, DDSCL_NORMAL);
   if (FAILED(hr)) {
 ...
 *  Compares the requested color depth with the desktop color depth.
 */
static int verify_color_depth_borderless (int color_depth)
{
   AL_CONST char *ddu;
   char tmp1[64], tmp2[128];
   int i;
   ...
   if ((gfx_directx_compare_color_depth(color_depth) == 0) && (color_depth != 8)) {
      /* the color depths match */ 
      _al_wd_update_window = update_matching_window_borderless;
   }
   else {
      /* disallow cross-conversion between 15-bit and 16-bit colors */
      if ((BYTES_PER_PIXEL(color_depth) == 2) && (BYTES_PER_PIXEL(_win_desktop_depth) == 2))
      ...
      _al_wd_update_window = update_colorconv_window_borderless;
 ...
 *  Sets the driver description string.
 */
static void gfx_directx_setup_driver_desc_borderless(void)
{
   char tmp1[128], tmp2[128];
   ...
   gfx_directx_win_borderless.desc = gfx_driver_desc;
}
...
static struct BITMAP *init_directx_win_borderless(int w, int h, int v_w, int v_h, int color_depth)
{
   unsigned char *cmap;
   HRESULT hr;
   int i;
   HWND allegro_wnd = win_get_window();
	...
	//SetWindowPos(allegro_wnd, HWND_TOP, 0, 0, w, h, SWP_NOZORDER);
	//SetWindowLong(allegro_wnd, GWL_STYLE, WS_POPUP);
	SetWindowLong(allegro_wnd, GWL_STYLE, WS_VISIBLE);
	SetWindowLong(allegro_wnd, GWL_EXSTYLE, WS_EX_APPWINDOW);
	SetWindowPos(allegro_wnd, HWND_TOP, 0, 0, w, h, SWP_NOZORDER | SWP_NOSIZE | SWP_FRAMECHANGED | SWP_NOMOVE);
   ...
   if (init_directx() != 0)
      goto Error;
   if (verify_color_depth_borderless(color_depth)) {
      ustrzcpy(allegro_error, ALLEGRO_ERROR_SIZE, get_config_text("Unsupported color depth"));
      goto Error;
   }
   if (wnd_call_proc(wnd_set_windowed_coop) != 0)
   ...
   if (_win_desktop_depth == 8) {
      ...
      if (color_depth == 8) {
         gfx_directx_win_borderless.set_palette = gfx_directx_set_palette_win_8;
      }
      else {
         cmap = _get_colorconv_map();
      ...
      if (color_depth == 8) {
         gfx_directx_win_borderless.set_palette = gfx_directx_set_palette_win;
   ...
   /* setup Allegro gfx driver */
   gfx_directx_setup_driver_desc_borderless();
   if (gfx_directx_setup_driver(&gfx_directx_win_borderless, w, h, color_depth) != 0)
      goto Error;

   gfx_directx_forefront_bitmap = gfx_directx_make_bitmap_from_surface(offscreen_surface, w, h, BMP_ID_VIDEO);

   gfx_directx_enable_acceleration(&gfx_directx_win_borderless);
   memcpy(&_special_vtable, &_screen_vtable, sizeof (GFX_VTABLE));
   _special_vtable.release = gfx_directx_unlock_win;
   _special_vtable.unwrite_bank = gfx_directx_unwrite_bank_win;
   gfx_directx_forefront_bitmap->vtable = &_special_vtable;
   ...
   /* connect to the system driver */
   win_gfx_driver = &win_gfx_driver_windowed_borderless;
