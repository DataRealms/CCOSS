...
static int key_dinput_init(void)
{
...
#ifdef _DEBUG
   hr = IDirectInput8_CreateDevice(key_dinput, &GUID_SysKeyboardEm2, &key_dinput_device, NULL);
#else
   ...
   hr = IDirectInputDevice8_SetCooperativeLevel(key_dinput_device, allegro_wnd, DISCL_FOREGROUND | DISCL_NONEXCLUSIVE);
   if (FAILED(hr))
      goto Error;
