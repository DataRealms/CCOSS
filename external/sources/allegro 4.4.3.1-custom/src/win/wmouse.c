...
static int mouse_dinput_init(void)
{
...
#ifdef _DEBUG
   hr = IDirectInput8_CreateDevice(mouse_dinput, &GUID_SysMouseEm2, &mouse_dinput_device, NULL);
#else
   ...
   hr = IDirectInputDevice8_SetCooperativeLevel(mouse_dinput_device, allegro_wnd, DISCL_FOREGROUND | DISCL_NONEXCLUSIVE);
   if (FAILED(hr))
	   goto Error;
